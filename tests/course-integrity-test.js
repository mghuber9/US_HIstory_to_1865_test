...
for(const s of sections){if(!s.id||!s.title||!Array.isArray(s.learnScreens)||!Array.isArray(s.questions))errors.push(`Malformed section ${s.id}`);const cfu=new Set(s.learnScreens.flatMap(x=>x.cfu||[]));for(const q of s.questions){const key=`${s.unit}:${s.id}:${q.id}`;if(ids.has(key))errors.push(`Duplicate ${key}`);ids.add(key);if(!Array.isArray(q.choices)||q.choices.length!==4||q.answer<0||q.answer>3||!q.choices[q.answer])errors.push(`Malformed ${key}`);if(s.unit>1&&!cfu.has(q.id))errors.push(`Question absent from Learn checks ${key}`)}}
